...
class BloodConnectApp {
    ...
    async checkAuthStatus() {
        ...
        try {
            ...
            // Prefer donor dashboard if donor profile exists
    ...
    initDonorDashboard() {
        ...
        profileDetails.innerHTML = `
            ...
            <div class="profile-row">
                ...
                <span>${typeof this.currentDonor.reliability === 'number' ? Math.round(this.currentDonor.reliability * 100) + '%' : '—'}</span>
            </div>
        `;
        ...
        if (upRecentList) {
            if (donations.length === 0) {
                ...
                upRecentList.innerHTML = donations
                    .slice(-5)
                    .map(d => `<div class="profile-row"><span>${new Date(d.date).toLocaleString()}</span><span>${d.type}</span></div>`)
                    .join('');
            }
        }

        // Wire record donation button if present
        const recBtn = document.getElementById('record-donation-btn');
        if (recBtn) {
            recBtn.onclick = () => {
                const newDonation = {
                    id: Date.now().toString(),
                    donorId: this.currentDonor.id,
                    type: 'Whole Blood',
                    date: new Date().toISOString()
                };
                this.mockData.donations.push(newDonation);
                this.saveDataToStorage();
                this.showToast('Donation Recorded', 'Your donation has been recorded', 'success');
                // Refresh dashboard stats and lists
                this.initDonorDashboard();
                const list = document.getElementById('donations-list');
                if (list) {
                    const items = this.mockData.donations.filter(d => d.donorId === this.currentDonor.id)
                        .sort((a,b) => new Date(b.date) - new Date(a.date))
                        .slice(0,5)
                        .map(d => `<div class="profile-row"><span>${new Date(d.date).toLocaleString()}</span><span>${d.type}</span></div>`)
                        .join('');
                    list.innerHTML = items || '<div class="empty-state"><p>No donations recorded yet</p></div>';
                }
            };
        }
    }
        ...
        if (this.searchResults.length > 0) {
            ...
            document.querySelectorAll('.contact-donor-btn').forEach(btn => {
                btn.addEventListener('click', () => {
                    ...
                    const donor = this.searchResults.find(d => String(d.id) === String(donorId));
                    if (donor) {
    ...
    updateSeekerRequestsList() {
        ...
        if (requestsListEl) {
            if (userRequests.length === 0) {
                ...
                requestsListEl.innerHTML = userRequests.map(request => {
    ...
    updateAcceptedRequestsList() {
        ...
        if (!listEl) return;
        const accepted = (this._requestsAsSeeker || []).filter(req => req.status === 'accepted');
            ...
            return `
                <div class="request-item">
                    <div class="request-item-header">
                        ...
                        <span class="request-status status-accepted">accepted</span>
                    </div>
                    <div class="request-details">
            ...
            if (donorRequests.length === 0) {
                        ...
                        <div class="request-item">
                            ...
                            <div class="request-actions">
                                ${request.status === 'pending' ? `
                                    ...
                                    <button class="btn btn-secondary reject-request-btn" data-request-id="${request.id}">
                                        Reject
                                    </button>
                                ` : ''}
                                <span class="request-detail-item" style="margin-left:auto; font-size:12px; color: var(--gray-600);">Updated: ${new Date(request.updatedAt || request.createdAt).toLocaleString()}</span>
                                <button class="btn btn-danger btn-small delete-request-btn" data-request-id="${request.id}">
                                    <svg class="icon-small" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                                    </svg>
                ...
                document.querySelectorAll('.accept-request-btn').forEach(btn => {
                    btn.addEventListener('click', () => {
                        ...
                        // After acceptance, update seeker accepted list if applicable
                        this.updateAcceptedRequestsList();
                    });
                });
